class VideoChatsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @video_chat.save
        NotificationMailer.delay.new_video_chat(@video_chat).deliver
  ...
  def update
    ...
    respond_to do |format|
      if @video_chat.update_attributes(video_chat_params)
        NotificationMailer.delay.video_chat_update(@video_chat, mail_recipient) if old_start_time != @video_chat.start_time || old_end_time != @video_chat.end_time.deliver
  ...
  def destroy
    ...
    NotificationMailer.delay.video_chat_cancellation(@video_chat, mail_recipient).deliver

    @video_chat.destroy

    respond_to do |format|
      format.html { redirect_to video_chats_url, notice: "Interview cancelled. Your interview partner has been notified." }
      format.json { head :no_content }
    end
  end
  ...
  def accept
    ...
    if @video_chat.save
      NotificationMailer.delay.accept_video_chat(@video_chat, mail_recipient).deliver
